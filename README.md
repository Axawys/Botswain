...
- **Локально** — агент поднимается на этом же ПК через локальный Docker (без
  SSH). Нужен установленный Docker и собранный образ агента
  (`cd agent && docker build -t botswain-agent:latest .`). Кнопка «Активировать
  агента локально» → сразу можно запускать ботов; «Отозвать агента» убирает
  контейнер агента.
- **SSH** — агент на VPS: ip/login/password → bootstrap → туннель. Есть кнопка
  «Отозвать агента» на сервере.

Режимы независимы: локальные боты и боты на VPS живут на разных Docker-демонах.

Когда агент активен, доступна секция **egress-прокси**: несколько прокси
(`http`/`https`/`socks5`), кнопка «Проверить прокси» помечает рабочие зелёным, а
нерабочие — красным. Первый рабочий становится активным, и новые боты ходят к
`api.telegram.org` через него (обход блокировок).
